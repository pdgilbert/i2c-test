...
//!
//! Sensor consumes a delay so these delays are generated with AltDelay.
...
fn main() -> ! {
    ...
    for  prt in parts {
       screen[0].clear();
       //hprintln!("prt {}", i);
       match Aht20::new(prt, DEFAULT_I2C_ADDRESS, AltDelay{}) {
           Ok(v)    => {sensors[i] = Some(v);
                        //write!(screen[0], "J{} in use", i+1).unwrap();
                       },
           Err(_e)  => {//write!(screen[0], "J{} unused", i+1).unwrap();
                       },
       }
                              ...
                              match sens.measure() {
                                   Ok(m)      => {//hprintln!("{} deg C, {}% RH", t.celsius(), h.rh()).unwrap();
                                                  write!(screen[ln], "J{} {:.1} {:.1}",
                                                          i+1, m.temperature.celcius(), m.relative_humidity).unwrap();
                                                 },
