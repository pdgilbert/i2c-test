...
//!
//! Sensor crates consumes a delay so these delays are generated with AltDelay.
...
fn main() -> ! {
    ...
    for  prt in parts {
       ...
       // Note: this sensor likes to be rebooted after it has failed. It gets into a bad state.
       //      Be sure to power off/on (unplug dongle) when testing code changes or next will fail.
       ...
       match th {
           ...
           Err(_e)  => {//hprintln!("single_measurement() error.");
                       },
       };
